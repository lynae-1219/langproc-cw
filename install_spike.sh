...
sudo apt-get update && sudo apt-get install -y --fix-missing \
    ...
    clang \
    bear \
    libboost-all-dev  # Required for Spike
...
sudo rm -rf riscv-gnu-toolchain.tar.gz riscv-isa-sim riscv-pk
...
# Install Spike RISC-V ISA Simulator (using a newer version to avoid header issues)
cd $WORKDIR
git clone https://github.com/riscv-software-src/riscv-isa-sim.git
cd riscv-isa-sim
git checkout master  # Use latest stable version instead of v1.1.0
mkdir build
cd build
